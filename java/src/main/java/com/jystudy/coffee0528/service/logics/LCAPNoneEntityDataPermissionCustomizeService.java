    ...
    public List<com.netease.lowcode.annotation.datapermission.common.structure.ResourceDataPermission> lCAPNoneEntityDataPermission(List<String> entityNames, String permissiontype) {
        ...
        if (CommonFunctionUtil.equals(permissiontype, "notLogin")) {
            for (Long index = 0L; index < entityNames.size(); index++) {
                ...
                resDataPermission.dataPermissions = CommonFunctionUtil.newWithInitiation(new ArrayList<com.netease.lowcode.annotation.datapermission.common.structure.DataPermission>(), _list566 -> {
    _list566.add(dataPermission);
} ); 
            ...
            for (Long index = 0L; index < entityNames.size(); index++) {
                ...
                resDataPermission.dataPermissions = CommonFunctionUtil.newWithInitiation(new ArrayList<com.netease.lowcode.annotation.datapermission.common.structure.DataPermission>(), _list234 -> {
    _list234.add(dataPermission);
} ); 
                resDataPermission.resourceType = "entity"; 
                resDataPermission.resourceName = item; 
