...
import com.jystudy.coffee0528.repository.LCAPGetLogicDataPermissionsCustomizeServiceMapper; 
import com.jystudy.coffee0528.domain.entities.LCAPColumnRule; 

@Service
public class LCAPGetLogicDataPermissionsCustomizeService {

    private static final Logger LCAP_LOGGER = LoggerFactory.getLogger(Constants.LCAP_CUSTOMIZE_LOGGER);
    @Autowired
    private LCAPGetLogicDataPermissionsCustomizeServiceMapper lCAPGetLogicDataPermissionsCustomizeServiceMapper;
    private ObjectMapper objectMapper = new ObjectMapper();
    @Autowired
    private LCAPNoneLogicDataPermissionCustomizeService lCAPNoneLogicDataPermissionCustomizeService;
    @Autowired
    ...
    public List<com.netease.lowcode.annotation.datapermission.common.structure.ResourceDataPermission> lCAPGetLogicDataPermissions(List<String> logicNames) {
        ...
        if (CommonFunctionUtil.hasValue(GlobalContext.getSessionVariable().currentUser, logicNames)) {
            userRole = lCAPGetLogicDataPermissionsCustomizeServiceMapper.getLong(GlobalContext.getSessionVariable().currentUser); 
            if (CommonFunctionUtil.hasValue(userRole)) {
                ...
                dataPerList = lCAPGetLogicDataPermissionsCustomizeServiceMapper.getAnonymousStructure_B347BE96D1C3E30B743C7EF6D26982A5(userRole, logicNames); 
                if (CommonFunctionUtil.hasValue(dataPerList)) {
                    ...
                    CommonFunctionUtil.addAll(resDataPermissionList, lCAPNoneLogicDataPermissionCustomizeService.lCAPNoneLogicDataPermission(nonDataPerList, "default"));
                    for (Long index = 0L; index < filterDataPerNameList.size(); index++) {
                        ...
                        for (Long index1 = 0L; index1 < roleDataPermissionList.size(); index1++) {
                            ...
                            if ((CommonFunctionUtil.hasValue(item1.lCAPDataPermission.rowRuleType) && (CommonFunctionUtil.equals(item1.lCAPDataPermission.rowRuleType, "self")))) {
                                ...
                                rowRuleItem.values = CommonFunctionUtil.newWithInitiation(new ArrayList<String>(), _list196 -> {
    _list196.add(GlobalContext.getSessionVariable().currentUser.userName);
} ); 
                                        ...
                                        for (Long index2 = 0L; index2 < rowRuleList.size(); index2++) {
                                            ...
                                            if ((CommonFunctionUtil.hasValue(item2.valuesType) && (CommonFunctionUtil.equals(item2.valuesType, "登录信息变量")))) {
                                                if ((CommonFunctionUtil.equals(CommonFunctionUtil.listHead(item2.values), "登录用户名"))) {
                                                    ...
                                                    rowRuleItem.values = CommonFunctionUtil.newWithInitiation(new ArrayList<String>(), _list621 -> {
    _list621.add(GlobalContext.getSessionVariable().currentUser.userName);
} ); 
                                                ...
                                                if ((CommonFunctionUtil.equals(CommonFunctionUtil.listHead(item2.values), "登录用户ID"))) {
                                                    rowRuleItem.comparison = item2.comparison; 
                                                    rowRuleItem.propertyName = item2.propertyName; 
                                                    rowRuleItem.values = CommonFunctionUtil.newWithInitiation(new ArrayList<String>(), _list838 -> {
    _list838.add(GlobalContext.getSessionVariable().currentUser.userId);
} ); 
                    ...
                    LCAP_LOGGER.info(CommonFunctionUtil.toString(new StringBuilder().append(CommonFunctionUtil.toString("逻辑数据权限：")).append(CommonFunctionUtil.toString(JacksonUtils.toJson(resDataPermissionList))).toString()));
                    result = resDataPermissionList; 
                } else {
                    LCAP_LOGGER.info(CommonFunctionUtil.toString("有角色，数据未赋予权限"));
                    result = lCAPNoneLogicDataPermissionCustomizeService.lCAPNoneLogicDataPermission(logicNames, "hasRole"); 
                } 

            } else {
                LCAP_LOGGER.info(CommonFunctionUtil.toString("无角色，数据无权限"));
                result = lCAPNoneLogicDataPermissionCustomizeService.lCAPNoneLogicDataPermission(logicNames, "notRole"); 
            } 

        } else {
            LCAP_LOGGER.info(CommonFunctionUtil.toString(new StringBuilder().append(CommonFunctionUtil.toString("未登录，数据无权限或无逻辑名集合：")).append(CommonFunctionUtil.toString(logicNames)).toString()));
            result = lCAPNoneLogicDataPermissionCustomizeService.lCAPNoneLogicDataPermission(logicNames, "notLogin"); 
        } 
