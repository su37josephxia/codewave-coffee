...
public class LoadCoffeeStructureListCustomizeService {
    ...
    public List<CoffeeStructureStructure> loadCoffeeStructureList() {
        ...
        if (CommonFunctionUtil.hasValue(coffeeList)) {
            result = CommonFunctionUtil.listTransform(coffeeList, (item) -> CommonFunctionUtil.newWithInitiation(new CoffeeStructureStructure(), _bean631 -> {
    _bean631.coffee = item.coffee; 
    _bean631.count = 0L; 
} )); 
        } else {
        } 
