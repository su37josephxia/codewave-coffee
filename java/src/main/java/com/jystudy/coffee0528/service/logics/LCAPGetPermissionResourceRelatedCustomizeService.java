    ...
    public List<LCAPPermissionAndResourceStructure> lCAPGetPermissionResourceRelated(Long permissinId) {
        ...
        if ((CommonFunctionUtil.notEquals(permissinId, null))) {
            ...
            if ((CommonFunctionUtil.length(variable1).compareTo(0L) > 0)) {
                result = CommonFunctionUtil.listTransform(variable1, (item) -> CommonFunctionUtil.newWithInitiation(new LCAPPermissionAndResourceStructure(), _bean235 -> {
    _bean235.text = new StringBuilder().append(CommonFunctionUtil.toString(item.lCAPResource.description)).append(CommonFunctionUtil.toString("（")).append(CommonFunctionUtil.toString(item.lCAPResource.name)).append(CommonFunctionUtil.toString("）")).toString(); 
    _bean235.value = item.lCAPResource.id; 
} )); 
            } else {
                result = CommonFunctionUtil.newWithInitiation(new ArrayList<LCAPPermissionAndResourceStructure>(), _list786 -> {}); 
            } 
