...
import com.jystudy.coffee0528.repository.entities.CoffeeOrderMappingEntityMapper;
...
/**
* auto generate CoffeeOrderMappingEntityService ServiceImpl
*
* @author sys
*/
@Service
public class CoffeeOrderMappingEntityService extends AbstractService {
    @Resource
    private CoffeeOrderMappingEntityMapper mapper;
    @Resource
    ...
    public CoffeeOrderMappingEntityService() {
        ...
        entityFieldNameTitleMap.put("orderId", "订单id");
        entityFieldTitleNameMap.put("订单id", "orderId");
        entityFiledColumnNameMap.put("orderId", "order_id");
        entityFieldNameTitleMap.put("coffeeId", "咖啡id");
        entityFieldTitleNameMap.put("咖啡id", "coffeeId");
        entityFiledColumnNameMap.put("coffeeId", "coffee_id");
        entityFieldNameTitleMap.put("coffeeQuantity", "下单数量");
        entityFieldTitleNameMap.put("下单数量", "coffeeQuantity");
        entityFiledColumnNameMap.put("coffeeQuantity", "coffee_quantity");
    }
    ...
    **/
    ...
    public CoffeeOrderMappingEntity create(CoffeeOrderMappingEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<CoffeeOrderMappingEntity> batchCreate(List<CoffeeOrderMappingEntity> entities) {
        ...
        // fill default value
        List<CoffeeOrderMappingEntity> batchList = new ArrayList<>(100);
        for (CoffeeOrderMappingEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public CoffeeOrderMappingEntity update(CoffeeOrderMappingEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "CoffeeOrderMappingEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<CoffeeOrderMappingEntity> batchUpdate(List<CoffeeOrderMappingEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<CoffeeOrderMappingEntity> updateEntities = new ArrayList<>(entities.size());
        for (CoffeeOrderMappingEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        CoffeeOrderMappingEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("CoffeeOrderMapping", "coffee_order_mapping", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<CoffeeOrderMappingEntity>  entities = mapper.selectList(inQueryFilter);
        for (CoffeeOrderMappingEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public CoffeeOrderMappingEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<CoffeeOrderMappingEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<CoffeeOrderMappingEntity> data = ExcelUtil.read(file.getInputStream(), type, CoffeeOrderMappingEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<CoffeeOrderMappingEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, CoffeeOrderMappingEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + CoffeeOrderMappingEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public CoffeeOrderMappingEntity createOrUpdate(CoffeeOrderMappingEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, CoffeeOrderMappingEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            CoffeeOrderMappingEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(CoffeeOrderMappingEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
