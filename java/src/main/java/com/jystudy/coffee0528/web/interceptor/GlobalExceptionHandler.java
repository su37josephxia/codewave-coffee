...
import com.jystudy.coffee0528.util.TraceIdHolder;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiReturn> handleMybatisSystemException(MyBatisSystemException ex) {
        Optional<Throwable> annotationSQLHandlerEx = Optional.ofNullable(ex)
        ...
        if (annotationSQLHandlerEx.isPresent()) {
            return ResponseEntity
                    ...
                    .body(ApiReturn.of("", HttpStatus.INTERNAL_SERVER_ERROR.value(), annotationSQLHandlerEx.get().getMessage(), ""));
        }
