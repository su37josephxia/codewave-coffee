...
public class LCAPGetPermissionResourseNotRelatedCustomizeService {
    ...
    public List<LCAPPermissionAndResourceStructure> lCAPGetPermissionResourseNotRelated(Long permissionId) {
        ...
        result = CommonFunctionUtil.listTransform(variable1, (item) -> CommonFunctionUtil.newWithInitiation(new LCAPPermissionAndResourceStructure(), _bean905 -> {
    _bean905.text = new StringBuilder().append(CommonFunctionUtil.toString(item.lCAPResource.description)).append(CommonFunctionUtil.toString("（")).append(CommonFunctionUtil.toString(item.lCAPResource.name)).append(CommonFunctionUtil.toString("）")).toString(); 
    _bean905.value = item.lCAPResource.id; 
} )); 
        return result;
    } 
