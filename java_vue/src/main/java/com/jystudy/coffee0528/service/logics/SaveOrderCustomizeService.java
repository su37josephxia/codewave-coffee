    ...
    public Boolean saveOrder(OrderEntityEntity order) {
        ...
        if (CommonFunctionUtil.hasValue(orderWrapper.param)) {
            ...
            if (CommonFunctionUtil.hasValue(createOrderResult.param.id)) {
                coffeeOrderMappingList = CommonFunctionUtil.listTransform(orderWrapper.param.coffeeList, (item) -> CommonFunctionUtil.newWithInitiation(new CoffeeOrderMappingEntity(), _bean499 -> {
    _bean499.orderId = createOrderResult.param.id; 
    _bean499.coffeeId = item.coffee.id; 
    _bean499.coffeeQuantity = item.count; 
} )); 
