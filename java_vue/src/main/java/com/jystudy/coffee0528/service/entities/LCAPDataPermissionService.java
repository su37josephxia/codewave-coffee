...
import com.jystudy.coffee0528.repository.entities.LCAPDataPermissionMapper;
...
/**
* auto generate LCAPDataPermissionService ServiceImpl
*
* @author sys
*/
@Service
public class LCAPDataPermissionService extends AbstractService {
    @Resource
    private LCAPDataPermissionMapper mapper;
    @Resource
    ...
    public LCAPDataPermissionService() {
        ...
        entityFieldNameTitleMap.put("resourceName", "所属实体名或逻辑名");
        entityFieldTitleNameMap.put("所属实体名或逻辑名", "resourceName");
        entityFiledColumnNameMap.put("resourceName", "resource_name");
        entityFieldNameTitleMap.put("resourceType", "所属资源类型");
        entityFieldTitleNameMap.put("所属资源类型", "resourceType");
        entityFiledColumnNameMap.put("resourceType", "resource_type");
        entityFieldNameTitleMap.put("rowRuleType", "行权限类型");
        entityFieldTitleNameMap.put("行权限类型", "rowRuleType");
        entityFiledColumnNameMap.put("rowRuleType", "row_rule_type");
        entityFieldNameTitleMap.put("relation", "行权限自定义规则间的逻辑关系");
        entityFieldTitleNameMap.put("行权限自定义规则间的逻辑关系", "relation");
        entityFiledColumnNameMap.put("relation", "relation");
        entityFieldNameTitleMap.put("roleId", "所属角色ID");
        entityFieldTitleNameMap.put("所属角色ID", "roleId");
    ...
    **/
    ...
    public LCAPDataPermission create(LCAPDataPermission entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPDataPermission> batchCreate(List<LCAPDataPermission> entities) {
        ...
        // fill default value
        List<LCAPDataPermission> batchList = new ArrayList<>(100);
        for (LCAPDataPermission entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public LCAPDataPermission update(LCAPDataPermission entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "LCAPDataPermission");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPDataPermission> batchUpdate(List<LCAPDataPermission> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<LCAPDataPermission> updateEntities = new ArrayList<>(entities.size());
        for (LCAPDataPermission entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        LCAPDataPermission entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("LCAPDataPermission", "lcap_data_permission_f803ac", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<LCAPDataPermission>  entities = mapper.selectList(inQueryFilter);
        for (LCAPDataPermission entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public LCAPDataPermission get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<LCAPDataPermission> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<LCAPDataPermission> data = ExcelUtil.read(file.getInputStream(), type, LCAPDataPermission.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<LCAPDataPermission> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, LCAPDataPermission.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + LCAPDataPermission.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public LCAPDataPermission createOrUpdate(LCAPDataPermission entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, LCAPDataPermission.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            LCAPDataPermission existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(LCAPDataPermission entity, List<String> updateFields, AbstractQueryFilter filter) {
