    ...
    public List<com.netease.lowcode.annotation.datapermission.common.structure.ResourceDataPermission> lCAPNoneLogicDataPermission(List<String> logicNames, String permissiontype) {
        ...
        if (CommonFunctionUtil.equals(permissiontype, "notLogin")) {
            for (Long index = 0L; index < logicNames.size(); index++) {
                ...
                resDataPermission.dataPermissions = CommonFunctionUtil.newWithInitiation(new ArrayList<com.netease.lowcode.annotation.datapermission.common.structure.DataPermission>(), _list992 -> {
    _list992.add(dataPermission);
} ); 
            ...
            for (Long index = 0L; index < logicNames.size(); index++) {
                ...
                resDataPermission.dataPermissions = CommonFunctionUtil.newWithInitiation(new ArrayList<com.netease.lowcode.annotation.datapermission.common.structure.DataPermission>(), _list798 -> {
    _list798.add(dataPermission);
} ); 
                resDataPermission.resourceType = "logic"; 
                resDataPermission.resourceName = item; 
