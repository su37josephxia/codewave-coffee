...
import com.jystudy.coffee0528.repository.entities.LCAPEntityMetaMapper;
...
/**
* auto generate LCAPEntityMetaService ServiceImpl
*
* @author sys
*/
@Service
public class LCAPEntityMetaService extends AbstractService {
    @Resource
    private LCAPEntityMetaMapper mapper;
    @Resource
    ...
    public LCAPEntityMetaService() {
        ...
        entityFieldNameTitleMap.put("entityName", "实体名");
        entityFieldTitleNameMap.put("实体名", "entityName");
        entityFiledColumnNameMap.put("entityName", "entity_name");
        entityFieldNameTitleMap.put("tableName", "实体的表名");
        entityFieldTitleNameMap.put("实体的表名", "tableName");
        entityFiledColumnNameMap.put("tableName", "table_name");
        entityFieldNameTitleMap.put("entityDescription", "实体的描述");
        entityFieldTitleNameMap.put("实体的描述", "entityDescription");
        entityFiledColumnNameMap.put("entityDescription", "entity_description");
        entityFieldNameTitleMap.put("properties", "属性列表");
        entityFieldTitleNameMap.put("属性列表", "properties");
        entityFiledColumnNameMap.put("properties", "properties");
    }
    ...
    **/
    ...
    public LCAPEntityMeta create(LCAPEntityMeta entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPEntityMeta> batchCreate(List<LCAPEntityMeta> entities) {
        ...
        // fill default value
        List<LCAPEntityMeta> batchList = new ArrayList<>(100);
        for (LCAPEntityMeta entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public LCAPEntityMeta update(LCAPEntityMeta entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "LCAPEntityMeta");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPEntityMeta> batchUpdate(List<LCAPEntityMeta> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<LCAPEntityMeta> updateEntities = new ArrayList<>(entities.size());
        for (LCAPEntityMeta entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        LCAPEntityMeta entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("LCAPEntityMeta", "lcap_entity_meta_f803ac", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<LCAPEntityMeta>  entities = mapper.selectList(inQueryFilter);
        for (LCAPEntityMeta entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public LCAPEntityMeta get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<LCAPEntityMeta> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<LCAPEntityMeta> data = ExcelUtil.read(file.getInputStream(), type, LCAPEntityMeta.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<LCAPEntityMeta> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, LCAPEntityMeta.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + LCAPEntityMeta.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public LCAPEntityMeta createOrUpdate(LCAPEntityMeta entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, LCAPEntityMeta.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            LCAPEntityMeta existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(LCAPEntityMeta entity, List<String> updateFields, AbstractQueryFilter filter) {
