...
import com.jystudy.coffee0528.repository.entities.OrderEntityEntityMapper;
...
/**
* auto generate OrderEntityEntityService ServiceImpl
*
* @author sys
*/
@Service
public class OrderEntityEntityService extends AbstractService {
    @Resource
    private OrderEntityEntityMapper mapper;
    @Resource
    ...
    public OrderEntityEntityService() {
        ...
        entityFieldNameTitleMap.put("totalAmount", "订单总金额");
        entityFieldTitleNameMap.put("订单总金额", "totalAmount");
        entityFiledColumnNameMap.put("totalAmount", "total_amount");
        entityFieldNameTitleMap.put("coffeeCount", "咖啡总数量");
        entityFieldTitleNameMap.put("咖啡总数量", "coffeeCount");
        entityFiledColumnNameMap.put("coffeeCount", "coffee_count");
        entityFieldNameTitleMap.put("orderStatus", "订单状态");
        entityFieldTitleNameMap.put("订单状态", "orderStatus");
        entityFiledColumnNameMap.put("orderStatus", "order_status");
        entityFieldNameTitleMap.put("consigneeName", "收货人姓名");
        entityFieldTitleNameMap.put("收货人姓名", "consigneeName");
        entityFiledColumnNameMap.put("consigneeName", "consignee_name");
        entityFieldNameTitleMap.put("consigneePhone", "收货人电话");
        entityFieldTitleNameMap.put("收货人电话", "consigneePhone");
        entityFiledColumnNameMap.put("consigneePhone", "consignee_phone");
        entityFieldNameTitleMap.put("consigneeArea", "收货人地区");
        entityFieldTitleNameMap.put("收货人地区", "consigneeArea");
        entityFiledColumnNameMap.put("consigneeArea", "consignee_area");
        entityFieldNameTitleMap.put("consigneeAddress", "收货人详细地址");
        entityFieldTitleNameMap.put("收货人详细地址", "consigneeAddress");
        entityFiledColumnNameMap.put("consigneeAddress", "consignee_address");
        entityFieldNameTitleMap.put("pickupMethod", "取货方式");
        entityFieldTitleNameMap.put("取货方式", "pickupMethod");
        entityFiledColumnNameMap.put("pickupMethod", "pickup_method");
        entityFieldNameTitleMap.put("remark", "备注");
        entityFieldTitleNameMap.put("备注", "remark");
        entityFiledColumnNameMap.put("remark", "remark");
        entityFieldNameTitleMap.put("coffeeList", "咖啡信息");
        entityFieldTitleNameMap.put("咖啡信息", "coffeeList");
        entityFiledColumnNameMap.put("coffeeList", "coffee_list");
    }
    ...
    **/
    ...
    public OrderEntityEntity create(OrderEntityEntity entity) {
        if (null == entity) {
        ...
        entity.setTotalAmount(getOrDefault(entity.getTotalAmount(), BigDecimal.ZERO));
        entity.setCoffeeCount(getOrDefault(entity.getCoffeeCount(), 0L));
        entity.setOrderStatus(getOrDefault(entity.getOrderStatus(), null));
        entity.setConsigneeName(getOrDefault(entity.getConsigneeName(), ""));
        entity.setConsigneePhone(getOrDefault(entity.getConsigneePhone(), ""));
        entity.setConsigneeArea(getOrDefault(entity.getConsigneeArea(), ""));
        entity.setConsigneeAddress(getOrDefault(entity.getConsigneeAddress(), ""));
        entity.setPickupMethod(getOrDefault(entity.getPickupMethod(), null));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OrderEntityEntity> batchCreate(List<OrderEntityEntity> entities) {
        ...
        // fill default value
        List<OrderEntityEntity> batchList = new ArrayList<>(100);
        for (OrderEntityEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public OrderEntityEntity update(OrderEntityEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "OrderEntityEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OrderEntityEntity> batchUpdate(List<OrderEntityEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<OrderEntityEntity> updateEntities = new ArrayList<>(entities.size());
        for (OrderEntityEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        OrderEntityEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("OrderEntity", "order_entity", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<OrderEntityEntity>  entities = mapper.selectList(inQueryFilter);
        for (OrderEntityEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public OrderEntityEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<OrderEntityEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<OrderEntityEntity> data = ExcelUtil.read(file.getInputStream(), type, OrderEntityEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<OrderEntityEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, OrderEntityEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + OrderEntityEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public OrderEntityEntity createOrUpdate(OrderEntityEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, OrderEntityEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            OrderEntityEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(OrderEntityEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
